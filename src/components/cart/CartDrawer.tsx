...
export default function CartDrawer({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const paymentSurcharge = 0; // No hay recargo ya que todos los métodos usan WhatsApp
  const finalTotal = cartSubtotal + shippingCost + paymentSurcharge;

  const getPaymentMethodLabel = (method: string) => {
    switch (method) {
      case 'mercadopago':
        return 'Mercado Pago (Tarjetas, transferencias, efectivo)';
      case 'cash':
  ...
  const handleWhatsAppOrder = async () => {
    ...
    try {
      ...
      const orderData = {
        ...
        payment_surcharge: 0, // No hay recargo
